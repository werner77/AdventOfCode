...
private val wires = listOf<Char>('a','b','c','d','e','f','g')
...
typealias WireMapping = Map<Char, Char>
...
private fun decode(scrambled: Pair<List<String>, List<String>>) : Int {
    ...
    return permutateUnique(wires.toSet()) { permutation ->
        // Convert the permutation to a mapping
        val mapping: WireMapping = permutation.foldIndexed(mutableMapOf()) { index, map, value ->
            map.apply { this[wires[index]] = value }
        }
