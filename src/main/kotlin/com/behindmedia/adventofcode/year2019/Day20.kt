...
class Day20 {
        ...
        fun isReachable(level: Int, recursiveMode: Boolean): Boolean {
            ...
            if (portal != null) {
                if (portal.connectedNode == null) {
                    // No connected portal, which means it is a global entry or exit, which is reachable com.behindmedia.adventofcode.year2018.only at level 0
                    return level == 0
                } else if (portal.outside && recursiveMode) {
                    // Outside portal, com.behindmedia.adventofcode.year2018.only reachable when level > 0
                    return level > 0
                }
            }
            return true
    ...
    private fun findMinimumSteps(start: Coordinate, end: Coordinate, map: Map<Coordinate, Node>, recursiveMode: Boolean): Int {
        ...
        return shortestPath(leveledStart, neighbours = { leveledCoordinate ->
            ...
            process = { nodePath ->
                if (nodePath.destination == leveledEnd) nodePath.length else null
            }
