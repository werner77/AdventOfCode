...
class Day18a {
    ...
    private fun constructWeightedGraph(input: String): Map<Node, List<Edge>> {
        ...
        for (baseNode in map.getNodes { it.identifier.isKey || it.identifier.isCurrentPosition }) {
            ...
            findAllKeyPaths(baseNode, map) { nodePath ->
                foundPaths[nodePath.node] = min(nodePath.pathLength, foundPaths[nodePath.node] ?: Int.MAX_VALUE)
            }
    ...
    /**
     * Returns all the neighbours of this node in a weighted graph.
     *
     * These are all possible paths to other key nodes with a weight (path length) attached.
     */
    private fun weightedNeighboursForNode(node: KeyedNode, graph: Map<Node, List<Edge>>): List<Pair<KeyedNode, Int>> {
        val edges = graph[node.node] ?: throw IllegalStateException("Could not find specified node ${node.node} in graph")
        return edges.mapNotNull { edge ->
            if (edge.isAvailable(node.keys)) Pair(KeyedNode(edge.to, node.keys + edge.to.identifier), edge.weight) else null
        }
    }

    /**
     * Method to find the minimum path in a weighted graph of nodes using Dijkstra's algorithm
     */
    private fun minimumPath(graph: Map<Node, List<Edge>>): Int? {
        val pending = PriorityQueue<NodePath<KeyedNode>>()
        val initialNodes = graph.keys.filter {
                it.identifier.isCurrentPosition
            }.map {
                NodePath(KeyedNode(it, KeyCollection.none), 0)
            }
        pending.addAll(initialNodes)
        val settled = mutableSetOf<KeyedNode>()
        while (true) {
            val current = pending.poll() ?: break
            val currentNode = current.node
            if (settled.contains(currentNode)) continue

            if (currentNode.keys.isComplete) {
