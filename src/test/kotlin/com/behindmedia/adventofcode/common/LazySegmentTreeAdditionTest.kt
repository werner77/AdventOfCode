...
class LazySegmentTreeAdditionTest {
    ...
    private class ValueNode(val value: Int) : SegmentNode<ValueNode, Int> {
        override fun plus(other: ValueNode) = ValueNode(this.value + other.value)
    }

    private class LazyAdditionNode(val value: Int) : LazySegmentNode<LazyAdditionNode, ValueNode, Int> {
        override fun plus(other: LazyAdditionNode) = LazyAdditionNode(this.value + other.value)

        override fun applyTo(
            node: ValueNode,
            range: IntRange
        ) = ValueNode(node.value + range.size * value)
    }

    private fun constructTree(arr: IntArray): LazySegmentTree<LazyAdditionNode, ValueNode, Int> {
        return LazySegmentTree(
            array = arr.toTypedArray(),
            nodeConstructor = ::ValueNode,
            lazyNodeConstructor = ::LazyAdditionNode)
    }
